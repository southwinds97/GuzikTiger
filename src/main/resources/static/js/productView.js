document.addEventListener('DOMContentLoaded', function () {
    ...
    const totalPriceElement = document.querySelector('#totalPrice .total strong em');
    const totalQuantityElement = document.querySelector('#totalPrice .total'); // (0개) 부분을 포함하는 요소
    const insufficientPriceElement = document.querySelector('.insufficientPrice'); // 33,000원 부분을 포함하는 요소
    ...
    function updateTotal() {
        ...
        if (insufficientPriceElement) {
            ...
            insufficientPriceElement.textContent = remainingPrice > 0 ? remainingPrice.toLocaleString() : '0';
            if (remainingPrice <= 0) {
        ...
        if (levelLineActiveElement) {
            const widthPercentage = Math.min((currentTotalPrice / basePrice) * 100, 100);
